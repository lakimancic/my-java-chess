...
public class King extends Figure {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                if (pos.y + j < 0 || pos.y + j >= 8)
                    continue;
                if (grid[pos.y + j][pos.x + i] != null && grid[pos.y + j][pos.x + i].getColor() == color)
                    continue;
    ...
    public boolean checkKingsideCastle() {
        ...
        return !isMoved &&
                ...
                grid[pos.y][pos.x + 3] != null && grid[pos.y][pos.x + 3].type == FigureType.ROOK
                && !grid[pos.y][pos.x + 3].isMoved &&
                !isPositionUnderAttack(new Position(pos.x + 1, pos.y), color, board)
                && !isPositionUnderAttack(new Position(pos.x + 2, pos.y), color, board);
    }
    ...
    public boolean checkQueensideCastle() {
        ...
        return !isMoved &&
                ...
                grid[pos.y][pos.x - 4] != null && grid[pos.y][pos.x - 4].type == FigureType.ROOK
                && !grid[pos.y][pos.x - 4].isMoved &&
                !isPositionUnderAttack(new Position(pos.x - 1, pos.y), color, board)
                && !isPositionUnderAttack(new Position(pos.x - 1, pos.y), color, board);
    }
    ...
    public void renderCheck(Graphics2D g, int tileSize) {
        if (isUnderCheck()) {
            Point2D center = new Point2D.Float(pos.x * tileSize + (float) tileSize / 2,
                    pos.y * tileSize + (float) tileSize / 2);
